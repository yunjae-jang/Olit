...
dag = DAG(
    dag_id='yvening_kakao',
    start_date= datetime(2024, 4, 2,11,0, tzinfo=kst), # DAG 시작 날짜
...
def excel(**context):
    ...
    df.to_csv('와이브닝채널고객_air.csv',index=False)
...
t1 = PythonOperator(task_id='get_id',
                    ...
                    op_args=['ca98ffc3e5f0d720d010b0ddd00fcf8d'],
...
def insertDB_record(**context):
    ...
    sql="INSERT INTO kakao_yv (customernum, phone, brand, relation, plustime, updatetime, foreigner) VALUES (%s, %s, %s, %s, %s, %s, %s)"
