...
dag = DAG(
    dag_id='dramang_kakao',
    start_date= datetime(2024, 4, 2,8,0, tzinfo=kst), # DAG 시작 날짜
...
def excel(**context):
    ...
    df.to_csv('닥터아망채널고객_air.csv',index=False)
...
t1 = PythonOperator(task_id='get_id',
                    ...
                    op_args=[ '484791aba5cfcb4e981290c619f86c7d'],
...
def insertDB_record(**context):
    ...
    sql="INSERT INTO kakao_dra (customernum, phone, brand, relation, plustime, updatetime, foreigner) VALUES (%s, %s, %s, %s, %s, %s, %s)"
