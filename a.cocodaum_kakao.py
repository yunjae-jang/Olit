...
dag = DAG(
    dag_id='cocodaum_kakao',
    start_date= datetime(2024, 4, 2,5,0, tzinfo=kst), # DAG 시작 날짜
...
def excel(**context):
    ...
    df.to_csv('코코채널고객_air.csv',index=False)
...
t1 = PythonOperator(task_id='get_id',
                    ...
                    op_args=['a46373bf765a7770c29acbf6c401dd6f'],
...
def insertDB_record(**context):
    ...
    sql="INSERT INTO kakao_co (customernum, phone, brand, relation, plustime, updatetime, foreigner) VALUES (%s, %s, %s, %s, %s, %s, %s)"
